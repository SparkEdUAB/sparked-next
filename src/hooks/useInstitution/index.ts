...
import {
  T_CreateInstitutionFields,
  T_FetchInstitutions,
  T_RawInstitutionFields,
  T_InstitutionFields,
  T_PublicInstitution,
} from './types';
...
export default function useInstitution() {
  ...
  const fetchInstitutions = useCallback(
    async (options: T_FetchInstitutions = {}) => {
      ...
      try {
        ...
        const _institutions =
          responseData.institutions?.map((i: T_RawInstitutionFields, index: number) =>
            transformRawInstitution(i, index),
          ) || [];
  ...
  const fetchPublicInstitutions = useCallback(async (search?: string, limit = 50) => {
  ...
  const deleteInstitutions = useCallback(async () => {
  ...
  const triggerEdit = useCallback(async () => {
    ...
    router.push(
      getChildLinkByKey('edit', ADMIN_LINKS.institutions) + `?institutionId=${String(selectedInstitutionIds[0])}`,
    );
