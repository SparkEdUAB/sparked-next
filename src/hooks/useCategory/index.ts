...
const useCategory = () => {
  ...
  const createCategory = useCallback(
    async (fields: T_CreateCategoryFields, onSuccessfullyDone?: () => void) => {
  ...
  const editCategory = useCallback(
    async (fields: T_CategoryFields, onSuccessfullyDone?: () => void) => {
  ...
  const fetchCategories = useCallback(
    async ({ limit = 1000, skip = 0 }: T_FetchCategorys) => {
  ...
  }, [message, selectedCategoryIds.length]);

  const deleteCategory = useCallback(
    async (items?: T_CategoryFields[]) => {
    ...
    [cartegories, message, selectedCategoryIds],
  );
  ...
  const findCategoryByName = useCallback(
    async ({ withMetaData = false }: { withMetaData: boolean }) => {
      ...
      const params = { name: searchQuery.trim(), limit: '1000', skip: '0', withMetaData: withMetaData.toString() };
